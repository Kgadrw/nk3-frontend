            ...
            {serviceCards.map((servicePair: any[], cardIndex: number) => {
              return (
                <div
                  ...
                  className="bg-[#009f3b] border border-[#009f3b] p-4 flex flex-col h-full flex-1"
                >
                  {servicePair.map((service: any, serviceIndex: number) => {
                    const key = service._id || service.id || `service-${cardIndex}-${serviceIndex}`;
                    return (
                      <div key={key} className={serviceIndex > 0 ? 'mt-4 pt-4 border-t border-white/20' : ''}>
                        ...
                        {service.features && service.features.length > 0 && (
                          <ul className="space-y-1 mb-2">
                            {service.features.map((feature: string, idx: number) => (
                              <li key={idx} className="flex items-start gap-2 text-white text-xs">
